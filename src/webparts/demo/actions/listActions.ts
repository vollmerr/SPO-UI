...
const getListsSuccess = (lists: IListItems[]): Action => ({
...
// export function getLists(spHttpClient: SPHttpClient, currentWebUrl: string, title: string) {
// 	return async (dispatch: any) => {

// 		dispatch(getListsRequest());

// 		try {
// 			const response: SPHttpClientResponse = await spHttpClient.get(`${currentWebUrl}/_api/web/lists/getByTitle('${title}')/items`, SPHttpClient.configurations.v1);
// 			const responseJSON = await response.json();
// 			// const lists: IODataList[] = responseJSON.value;
// 			const listItems: IListItems[] = responseJSON.value;
//       console.log("listItems: ", listItems);
// 			dispatch(getListsSuccess(listItems));
...
export function getLists(spHttpClient: SPHttpClient, currentWebUrl: string, title: string) {
  return (dispatch) => {
    ...
    return spHttpClient.get(`${currentWebUrl}/_api/web/lists/getByTitle('${title}')/items`, SPHttpClient.configurations.v1)
      .then(response => response.json())
      .then(json => json.value)
      .then(lists => dispatch(getListsSuccess(lists)))
